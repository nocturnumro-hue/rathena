...
bool status_calc_weight(map_session_data *sd, enum e_status_calc_weight_opt flag)
{
	...
	sd->max_weight = job_db.get_maxWeight(pc_mapid2jobid(sd->class_, sd->status.sex)) 
                 + sd->status.str * 300   // STR tăng 300 max weight
                 + sd->status.int_ * 150  // INT tăng 150 max weight
                 + sd->status.dex * 50;   // DEX tăng 50 max weight
...
static bool status_change_start_post_delay(block_list* src, block_list* bl, sc_type type, int32 val1, int32 val2, int32 val3, int32 val4, int32 tick, uint8 flag)
{
	...
	switch(type)
	{
		...
		case SC_AUTOTIME: // AUTO Status OFF ICON
		case SC_AUTOTIMEON: // AUTO Status ON ICON
		case SC_REBIRTH_REGEN: // VIP Rebirth Regen Icon		
...
TIMER_FUNC(status_change_timer){
	...
	case SC_AUTOATTACK:
		if (--(sce->val4) > 0) {
					...
					aa_check_item_pickup_onfloor(sd);
					{
						if (sd->aa.itempick_id) {
							...
							if (fitem_bl) {
								...
								//ShowError("distance %d \n",check_distance_bl(sd, fitem_bl, 2));
								if (!check_distance_bl(sd, fitem_bl, 2)) {
									if (sd->aa.prio_item_config || (!sd->aa.prio_item_config && !sd->aa.target_id))
										unit_walktobl(sd, fitem_bl, 1, 1);
									if (sd->aa.prio_item_config)
										skip = true;
								}
								else {
			...
			if (!sd->aa.prio_item_config || (sd->aa.prio_item_config && !sd->aa.itempick_id))
						...
						if (last_tick >= sd->aa.skill_cd) {
							for (auto& itAutoheal : sd->aa.autoheal) {
								if (aa_canuseskill(sd, itAutoheal.skill_id, itAutoheal.skill_lv) && ((status->hp * 100 / itAutoheal.min_hp) < sd->status.max_hp)) {
									...
									//ShowError("1 - itAutoheal.last_use %d, skill_delayfix %d, last_tick %d \n",itAutoheal.last_use,skill_delayfix(sd, itAutoheal.skill_id, itAutoheal.skill_lv),last_tick);
			...
			{//Sit to rest
				if (battle_config.feature_autoattack_sittorest) {
					if (sd->aa.autositregen.is_active) {
						...
						uint8 overweight = (overweight_percent >= battle_config.major_overweight_rate) ? 2 : (overweight_percent >= battle_config.natural_heal_weight_rate) ? 1 : 0;
							...
							for (auto& itAutoattackskills : skills_temp_list) {
								...
								if (last_tick >= sd->aa.skill_cd
									...
									if (skill_get_inf(itAutoattackskills.skill_id) & INF_ATTACK_SKILL || skill_get_inf(itAutoattackskills.skill_id) & INF_GROUND_SKILL) {
										...
										if (!check_distance_bl(sd, target, aa_skill_range) || !path_search_long(NULL, sd->m, sd->x, sd->y, target->x, target->y, CELL_CHKWALL)) {
											//ShowError("i need to walk %d \n", check_distance_bl(sd, target, skill_get_range(itAutoattackskills.skill_id, itAutoattackskills.skill_lv)));
			...
			if ((!sd->aa.prio_item_config || (sd->aa.prio_item_config && !sd->aa.itempick_id)) && sd->aa.target_id)
				aa_check_target_alive(sd);
	...
	case SC_REBIRTH_REGEN:
		if (!status_isdead(*bl)) {
			int32 hp_heal = status->max_hp * sce->val2 / 100;
			int32 sp_heal = status->max_sp * sce->val2 / 100;
			if (hp_heal > 0 || sp_heal > 0)
				status_heal(bl, hp_heal, sp_heal, 3);
			sc_timer_next(5000 + tick);
			return 0;
		}
		break;
	...
	if (!aa_check_item_pickup(sd, bl)){
		if( (!sd->aa.prio_item_config && !sd->aa.target_id)
			...
			for (i_ = 0; i_ < battle_config.feature_autoattack_pdetection; i_++){
				map_foreachinarea(buildin_autopick_sub, sd->m, sd->x - i_, sd->y - i_, sd->x + i_, sd->y + i_, BL_ITEM, &itempick_id_, sd->id);
				if (itempick_id_){
...
bool aa_check_target(map_session_data *sd, unsigned int id)
{
	...
	if (bl && path_search(NULL, sd->m, sd->x, sd->y, bl->x, bl->y, 0, CELL_CHKNOPASS) && distance_xy(sd->x, sd->y, bl->x, bl->y) < MAX_WALKPATH){
...
unsigned int aa_check_target_alive(map_session_data *sd){
	...
	if (!aa_check_target(sd, sd->aa.target_id)){
		...
		for (i_ = 0; i_ < battle_config.feature_autoattack_mdetection; i_++){
			...
			map_foreachinarea(buildin_autoattack_sub, sd->m, sd->x - i_, sd->y - i_, sd->x + i_, sd->y + i_, BL_MOB, &target_id_, sd->id);
			if (target_id_){
