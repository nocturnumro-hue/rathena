...
BUILDIN_FUNC(getautoattackstring)
{
	...
	case 4:	// auto attack skill desc
		if (index == -1 || index == -2) { // Show all attack skills available
			int count_skill = 0;
			for (i = 0; i < MAX_SKILL; i++) {
				if (sd->status.skill[i].id > 0 && sd->status.skill[i].lv > 0) {
					skill = skill_db.find(sd->status.skill[i].id);
...
+* makeitem <item id>,<amount>,"<map name>",<X>,<Y>{,<canShowEffect>{,<dropeffect>}};
+* makeitem "<item name>",<amount>,"<map name>",<X>,<Y>{,<canShowEffect>{,<dropeffect>}};
*/
BUILDIN_FUNC(makeitem) {
	...
	int32 m;
	struct item item_tmp;
	int16 dropeffect = -1; /* [royrdev] Drop Effect Item Floor */	
	...
	if( script_hasdata(st, 8) ) {
		dropeffect = static_cast<int16>(script_getnum(st, 8));
		if( dropeffect < -1 || dropeffect >= DROPEFFECT_MAX ) {
			ShowWarning("buildin_makeitem: Invalid dropeffect %d, using default.\n", dropeffect);
			dropeffect = -1;
		}
		if( dropeffect >= 0 )
			canShowEffect = true;
	...
	map_addflooritem(&item_tmp, amount, m, x, y, 0, 0, 0, 4, 0, canShowEffect, DIR_MAX, BL_NUL, dropeffect);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(is_itemfloor)
{
	...
	if( script_isstring(st, 2) ) {
		...
		if( id)
			...
			ShowError("buildin_itemfloor: Unknown item '%s'.\n", name);
			return SCRIPT_CMD_FAILURE;
		}
	}
	else {
		...
		if( !item_db.exists(nameid) ) {
			ShowError("buildin_itemfloor: Unknown item id '%u'.\n", nameid);
			return SCRIPT_CMD_FAILURE;
		}
	}
	amount = script_getnum(st, 3);
	...
	if( script_hasdata(st, 4) ) {
		dropeffect = static_cast<int16>(script_getnum(st, 4));
		if( dropeffect < -1 || dropeffect >= DROPEFFECT_MAX ) {
			ShowWarning("buildin_itemfloor: Invalid dropeffect %d, using default.\n", dropeffect);
			dropeffect = -1;
		}
	}
	if( script_hasdata(st, 5) ) {
		scatter = static_cast<int16>(script_getnum(st, 5));
		if( scatter < 0) scatter = 0;
		if( scatter > 5) scatter = 5;
	}
	...
	for( uint16 i = 0; i < amount; i++ ) {
		...
		int16 drop_y = base_y;
		if( scatter > 0 ) {
			drop_x += (rnd() % (scatter * 2 + 1)) - scatter;
			drop_y += (rnd() % (scatter * 2 + 1)) - scatter;
		}
		map_addflooritem(&item_tmp, 1, m, drop_x, drop_y, 0, 0, 0, 4, 0, canShowEffect, DIR_MAX, BL_NUL, dropeffect);
	}
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(is_getitemid)
{
	const char *name = script_getstr(st, 2);
	std::shared_ptr<item_data> id = item_db.searchname(name);

	if( id != nullptr )
		script_pushint(st, id->nameid);
	else
		script_pushint(st, 0);
...
BUILDIN_FUNC(makeitem2) {
	...
	if ((id = itemdb_search(nameid))) {
		...
		map_addflooritem(&item_tmp, amount, m, x, y, 0, 0, 0, 4, 0, canShowEffect, DIR_MAX, BL_NUL, -1);
	}
	else
		return SCRIPT_CMD_FAILURE;
	return SCRIPT_CMD_SUCCESS;
...
BUILDIN_FUNC(duplicatenpc)
{
	...
	const char* newname_str = script_getstr(st, 3);
	const char* mapname = script_getstr(st, 4);
	int16 x = (int16)script_getnum(st, 5);
	int16 y = (int16)script_getnum(st, 6);
	...
	struct npc_data* nd_source = npc_name2id(sourcename);
	if (!nd_source) {
		ShowError("duplicatenpc: source NPC '%s' not found.\n", sourcename);
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
	...
	int16 mapid = map_mapname2mapid(mapname);
	if (mapid < 0) {
		ShowError("duplicatenpc: map '%s' not found.\n", mapname);
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	struct npc_data* nd_target = npc_duplicate_npc(*nd_source, newname, mapid, x, y, class_, dir, -1, -1, nullptr);
	if (!nd_target) {
		ShowError("duplicatenpc: failed to duplicate NPC '%s'.\n", sourcename);
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
...
{
	if (!script_hasdata(st, 2)) {
		ShowError("duplicateremove: missing NPC name.\n");
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(is_itemfloor,"vi??"),
	BUILDIN_DEF(is_getitemid,"s"),
