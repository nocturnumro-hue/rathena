...
int32 battle_check_target( block_list *src, block_list *target,int32 flag)
{
	...
	// ========== CHECK FAKE PLAYER (mob 30101-30300) ==========
	if (target->type == BL_MOB) {
		TBL_MOB *md = (TBL_MOB *)target;
	
		if (md->mob_id >= 30101 && md->mob_id <= 30300) {
	
			// Trace back AoE skill unit to its real owner
			block_list *real_src = src;
			if (src->type == BL_SKILL) {
				skill_unit *su = (skill_unit*)src;
				if (su && su->group)
					real_src = map_id2bl(su->group->src_id);
			}
	
			// Players cannot attack fake players
			if (real_src && real_src->type == BL_PC)
				return 0;
	
			if (real_src && real_src->type == BL_MOB) {
				TBL_MOB *src_md = BL_CAST(BL_MOB, real_src);
				if (src_md) {
					// Fake players cannot attack each other
					if (src_md->mob_id >= 30101 && src_md->mob_id <= 30300)
						return 0;
	
					// These specific mobs cannot attack fake players
					switch (src_md->mob_id) {
						case 21835: // AI Rune Knight
						case 21836: // AI Royal Guard
						case 21837: // AI High Priest
						case 21838: // AI Hunter
						case 21839: // AI Guillotine Cross
						case 21840: // AI Shadow Chaser
						case 21841: // AI Warlock
						case 30049: // AI Cardinal						
							return 0;
						default:
							break; // Normal monsters are allowed to attack fake players
					}
				}
			}
		}
...
// rAthena
	...
	{ "equipment",              			&battle_config.equipment,           			0,      0,      MAX_REFINE,     },
	{ "weapon_level_1",              		&battle_config.weapon_level_1,           		0,      0,      MAX_REFINE,     },
	{ "weapon_level_2",              		&battle_config.weapon_level_2,           		0,      0,      MAX_REFINE,     },
